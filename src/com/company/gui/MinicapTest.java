...
public class MinicapTest extends JFrame {
	...
	public static final String PLATFORM_TOOLS_PATH = "/Users/wangzy/software/android-sdk-macosx/platform-tools/";

	public MinicapTest() {

		ADB adb = new ADB(PLATFORM_TOOLS_PATH);
		...
		mp = new MyPanel(device, this, PLATFORM_TOOLS_PATH);
...
//        pack();
	...
	private Point pointConvert(Point point) {
		Point realpoint = new Point((int) ((point.getX() * 1.0 / width) * banner.getMaxX()),
				(int) ((point.getY() * 1.0 / height) * banner.getMaxY()));
		return realpoint;
	}
	...
	class MyPanel extends JPanel implements AndroidScreenObserver {
		...
		public MyPanel(IDevice device, MinicapTest frame, String adbPath) {
			minicap = new MiniCapUtil(device, adbPath);
