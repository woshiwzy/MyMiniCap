...
public class MiniCapUtil implements ScreenSubject {
	...
	public MiniCapUtil(IDevice device, String adbRootPath) {
		this.device = device;
		this.ADB_PULL_COMMAND = adbRootPath + ADB_PULL_COMMAND;
		init();
	}

	// 判断是否支持minicap
	public boolean isSupoort() {
		String supportCommand = String.format("LD_LIBRARY_PATH=/data/local/tmp /data/local/tmp/minicap -P %s@%s/0 -t",
				size, size);
	...
	private void init() {
		...
		File minicapBinFile = new File(Constant.getMinicapBin(), abi + File.separator + MINICAP_FILE);
		File minitouchBinFile = new File(Constant.getMiniTouchBin(), abi + File.separator + MINITOUCH_FILE);
		File minicapSoFile = new File(Constant.getMinicapSo(),
				"android-" + sdk + File.separator + abi + File.separator + MINICAP_SO);
		System.out.println("minicap 路径:" + minicapSoFile.getAbsolutePath());
		try {
			
			// 将minicap的可执行文件和.so文件一起push到设备中
			//注意以下的推送过去的so 是不对的，以文档为准
			device.pushFile(minicapBinFile.getAbsolutePath(), REMOTE_PATH + File.separator + MINICAP_FILE);
			device.pushFile(minitouchBinFile.getAbsolutePath(), REMOTE_PATH + File.separator + MINITOUCH_FILE);
			device.pushFile(minicapSoFile.getAbsolutePath(), REMOTE_PATH + File.separator + MINICAP_SO);
			
			executeShellCommand(String.format(MINICAP_CHMOD_COMMAND, REMOTE_PATH, MINICAP_FILE));
			executeShellCommand(String.format(MINICAP_CHMOD_COMMAND, REMOTE_PATH, MINITOUCH_FILE));
			// 端口转发
			device.createForward(PORT, "minicap", DeviceUnixSocketNamespace.ABSTRACT);

			// 端口转发
			device.createForward(PORT2, "minitouch", DeviceUnixSocketNamespace.ABSTRACT);
	...
	public void takeScreenShotOnce() {
		...
		String takeScreenShotCommand = String.format(MINICAP_TAKESCREENSHOT_COMMAND, MINICAP_FILE, size, size,
				savePath);
		String localPath = System.getProperty("user.dir") + "/screenshot.jpg";
		String pullCommand = String.format(ADB_PULL_COMMAND, device.getSerialNumber(), savePath, localPath);
		System.out.println(getClass().getCanonicalName() + "执行adb命令:" + ADB_PULL_COMMAND);
	...
	class ImageBinaryFrameCollector implements Runnable {
		...
		public void run() {
			...
			try {
				final String startCommand = String.format(MINICAP_START_COMMAND, MINICAP_FILE, size, size);
				// 启动minicap服务
				...
				while (isRunning) {
					...
					if (buffer.length != realLen && realLen != -1) {
						buffer = subByteArray(buffer, 0, realLen);
						System.out.println("收到原始数据:" + realLen);
					}
	...
	class TouchThread implements Runnable {
		...
		public void run() {
			...
			System.out.println("MiniTouch 命令：" + startCmd);
				...
				for (int cursor = 0; cursor < len;) {
					...
					if (readBannerBytes < bannerLength) {
						...
						if (len - cursor >= frameBodyLength) {
							...
							if (frameBody.length > 0 && ((frameBody[0] != -1) || frameBody[1] != -40)) {
							...
							new Thread(new Runnable() {
								@Override
								public void run() {
									System.out.println("收到图数据大小：" + finalBytes.length);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.wuba.utils.screenshot.ScreenSubject#registerObserver(com.wuba.utils
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.wuba.utils.screenshot.ScreenSubject#removeObserver(com.wuba.utils
	 * .screenshot.AndroidScreenObserver)
	 */
